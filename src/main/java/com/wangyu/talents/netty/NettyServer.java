...
public class NettyServer {
  ...
  /**
   * 配置服务端的NIO线程组 NioEventLoopGroup 是用来处理I/O操作的Reactor线程组 bossGroup：用来接收进来的连接，workerGroup：用来处理已经被接收的连接,进行socketChannel的网络读写，
   * bossGroup接收到连接后就会把连接信息注册到workerGroup workerGroup的EventLoopGroup默认的线程数是CPU核数的二倍
   */
  public void bind(int port) {
    ...
    try {
       ...
       * 使用对象池，重用缓冲区
       */
      serverBootstrap = serverBootstrap.option(
          ChannelOption.RCVBUF_ALLOCATOR, new AdaptiveRecvByteBufAllocator(64, 10496, 1048576));
