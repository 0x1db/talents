angular.module('talents.serve', [])
.constant('mainUrl', '')  //暂不通过zuul代理目录
...
}).value('uiSelect2Config', {}).directive('uiSelect2',
                ...
                if (opts.initSelection) {
                  ...
                  opts.initSelection = function (element, callback) {
                    initSelection(element, function (value) {
                      ...
                      elm.prev().toggleClass('ng-pristine',
                          controller.$pristine);
                    });
                  };
                }
    ...
    function (mainUrl, $http, $cookies, $window, userRoles) {
      return function () {
        ...
        $.ajax(settings).done(function (res) {
          console.log('用户拥有的权限集合:', res);
          if (res.responseCode == '_200') {

            var resUrlAll = [];
            res.data.forEach(function (item) {
              resUrlAll.push(item.resource + '&' + item.methods);
            });
            console.log(resUrlAll);
            userRoles.roles = resUrlAll.toString();
            $window.sessionStorage['userPermissions'] = resUrlAll.toString();
          }
        });
