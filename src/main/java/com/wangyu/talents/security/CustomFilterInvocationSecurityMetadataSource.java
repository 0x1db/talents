...
public class CustomFilterInvocationSecurityMetadataSource implements
  ...
  public Collection<ConfigAttribute> getAttributes(Object object) throws IllegalArgumentException {
    ...
    int index = 0;
    String matchUrl = "";
    List<SystemResourceEntity> currentCompetences = null;
    try {
    ...
    for (SystemResourceEntity competence : currentCompetences) {
      String methods = competence.getMethods();
      if (methods.indexOf(method) != -1) {
        // 取得当前权限和角色的绑定关系
        List<SystemRoleEntity> roles = roleService.findByResourceId(competence.getId());
   ...
   */
  private List<SystemResourceEntity> queryByRequestUrl(String requestUrl) {
    List<SystemResourceEntity> competences = this.resourceService.findByResource(requestUrl);
